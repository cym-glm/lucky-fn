...
export function filterData(tableData, localRowCount) {
  const dataLength = tableData.length
  let j = 0
  for (let i = 0; i < dataLength; i++) {
    if (i % localRowCount === 0) j++
...
export function addOtherInfo(personList) {
...
export function selectCard(cardIndexArr, tableLength) {
  const cardIndex = Math.floor(Math.random() * (tableLength - 1))
  if (cardIndexArr.includes(cardIndex)) return selectCard(cardIndexArr, tableLength)
  return cardIndex
}
