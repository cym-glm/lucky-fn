...
import { usePositionConfig } from '@/store/positionConfig'
...
const { getAllPositionList: allPositionList, getNotPositionList: notPositionListPos } = storeToRefs(positionConfig)
const { getCurrentPrize: currentPrize } = storeToRefs(prizeConfig)
const { getTopTitle: topTitle, getCardColor: cardColor, getPatterColor: patternColor, getPatternList: patternList, getTextColor: textColor, getLuckyColor: luckyColor, getCardSize: cardSize, getTextSize: textSize, getRowCount: rowCount, getBackground: homeBackground, getIsShowAvatar: isShowAvatar, getPositionMode: positionMode } = storeToRefs(globalConfig)
...
function initTableData() {
  const allList = positionMode.value ? allPositionList.value : allPersonList.value
  if (allList.length <= 0) {
    return
  }
  const totalCount = rowCount.value * 7
  const originPersonData = JSON.parse(JSON.stringify(allList))
...
function startLottery() {
  ...
  personPool.value = positionMode.value ? notPositionListPos.value : (currentPrize.value.isAll ? notThisPrizePersonList.value : notPersonList.value)
  // 验证抽奖人数是否还够
...
async function continueLottery() {
  ...
  const customCount = currentPrize.value.separateCount
  if (!positionMode.value && customCount && customCount.enable && customCount.countList.length > 0) {
    for (let i = 0; i < customCount.countList.length; i++) {
      if (customCount.countList[i].isUsedCount < customCount.countList[i].count) {
  ...
  if (!positionMode.value) {
    ...
    luckyCount.value = 0
    positionConfig.addAlreadyPositionList(luckyTargets.value)
  }
  await enterLottery()
}
  ...
  <div id="container" ref="containerRef" class="3dContainer">
    ...
    <div id="menu">
      ...
      <div v-if="currentStatus === 3" class="flex justify-center gap-6 enStop">
        ...
        <div class="start">
          <button class="btn-start" @click="redrawLottery">
            <strong>{{ t('button.redraw') }}</strong>
            <div id="container-stars">
              <div id="stars" />
            </div>
            <div id="glow">
